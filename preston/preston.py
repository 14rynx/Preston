...
class Preston:
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.request_args = kwargs.get("request_args", {})
    ...
    def _get_access_from_refresh(self) -> Tuple[str, float]:
        ...
        r = self.session.post(self.TOKEN_URL, headers=headers, data=data, **self.request_args)
    ...
    def whoami(self) -> dict:
        ...
        if not self.access_token:
            return {}
        self._try_refresh_access_token()
        return self.session.get(self.WHOAMI_URL, **self.request_args).json()
    ...
    def get_path(self, path: str, data: dict) -> Tuple[dict, dict]:
        ...
        if cached_data:
            return cached_data
        self._try_refresh_access_token()
        resp = self.session.get(target_url, **self.request_args)
        self.cache.set(resp)
    ...
    def post_path(
        ...
        self._try_refresh_access_token()
        resp = self.session.post(target_url, json=post_data, **self.request_args)
        if resp.text:
            return resp.json()
        return None
    ...
    def delete_path(self, path: str, path_data: Union[dict, None]) -> dict:
        ...
        self._try_refresh_access_token()
        resp = self.session.delete(target_url, **self.request_args)
        if resp.text:
            return resp.json()
        return None
